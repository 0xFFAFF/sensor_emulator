...
class humEmulator:
    ...
    def read_data_content(self):
        ...
        filename = os.path.join(dirname, 'humidity.csv')
    ...
    def generate_date(self, queue):
        ...
        for i in data:
            ...
            item_raw = {'ID': ID, 'data_type':'humidity','sensor_data': sensor_data, 'time': now_time}
...
class Client():
    def __init__(self, serverIp, serverPort, humEmulator):
        self.serverIp = serverIp
        self.serverPort = serverPort
        self.clientSocket = socket(AF_INET, SOCK_STREAM)
        self.humEmulator = humEmulator
    ...
    def get_emulator_data(self):
        ...
        '''
        getDataThread = threading.Thread(target=self.humEmulator.generate_date, args=(self.queue,))
        getDataThread.start()
    ...
    def parse(self):
        ...
        if raw_data == 'stop':
            self.humEmulator.set_stage(STOP_STATE)
            print('正在设置stop状态')
        elif raw_data == 'start':
            self.humEmulator.set_stage(SEND_STATE)
        elif re.match('interval [0-9]+', raw_data) != None:
            self.humEmulator.set_interval(int(raw_data[8:]))
